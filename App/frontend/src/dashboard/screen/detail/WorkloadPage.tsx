    ...
    <div className="h-full overflow-y-auto p-6 space-y-4" style={{ fontFamily: "'Inter','Noto Sans KR',sans-serif" }}>
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {isLeader && (
            <button onClick={() => setShowAddTask(true)} className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-white rounded-lg" style={{ background: "linear-gradient(135deg,#7048E8,#4F6EF7)" }}><Plus className="w-3.5 h-3.5" />업무 배정</button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-card rounded-xl p-5 border border-border shadow-sm">
          ...
          <div className="space-y-4">
            {!summaryLoading && workload.map((entry, index) => {
              ...
              return (
                <div key={entry.assigneeId}>
                  <div className="flex items-center justify-between text-xs mb-1.5">
                    ...
                    <span className={`font-semibold ${pct >= 60 ? "text-slate-900" : pct >= 40 ? "text-amber-600" : "text-red-500"}`}>{pct}%</span>
                  </div>
            ...
            <div className="divide-y divide-border">
              {selectedTasks.map((task, index) => (
                <div key={task.id} className="flex items-center gap-3 px-5 py-3 hover:bg-muted/30 transition-colors">
                  ...
                  <button onClick={() => setDetailTarget(task)} className="text-[11px] font-medium text-blue-600 hover:text-blue-700 whitespace-nowrap">보드</button>
                </div>
              ))}
