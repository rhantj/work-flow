...
interface TaskDueDatePopupProps {
  task: DashboardTaskDto;
  projectId: number;
  onClose: () => void;
  onChanged: (newDueDate: string) => void;
}

export function TaskDueDatePopup({ task, projectId, onClose, onChanged }: TaskDueDatePopupProps) {
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4" onClick={e => e.stopPropagation()}>
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm" style={{ fontFamily: "'Inter','Noto Sans KR',sans-serif" }}>
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            <h2 className="text-base font-bold text-foreground">마감일 조정</h2>
