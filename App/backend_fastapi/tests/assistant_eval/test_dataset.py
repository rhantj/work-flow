...
#
# 코퍼스 스냅샷의 인물은 `구성원A` 처럼 뒤에 라틴 문자가 붙은 별칭이다. 한글만 잡으면 별칭이
# 하나도 안 걸려서 인물 규칙이 아무것도 안 하는 채로 통과한다.
SPEAKER_PATTERN = re.compile(r"(?:^|근거:\s*)(?!근거:)([가-힣]{2,4}[A-Z]?):")
ASSIGNEE_PATTERN = re.compile(r"(?<![가-힣])([가-힣]{2,4}[A-Z]?)[가이]\s*(?:담당|리뷰)")
