...
사람 이름은 전부 `구성원나`~`구성원터`(24명)로 치환했다. 출처가 두 갈래다.
...
별칭은 **순수 한글 4자**다. 처음에는 `구성원A` 처럼 라틴 문자를 붙였는데, 운영 코드
(`app/main.py`)의 이름 추출이 전부 `[가-힣]{2,4}` 라서 별칭을 통째로 못 본다. 회의 분석
픽스처에 같은 별칭을 쓰자 화자·담당자 후보가 빈 값이 되며 테스트 12건이 깨졌다 — 픽스처는
통과하는데 정작 테스트하려던 이름 추출은 아무것도 안 하는 상태가 된다.

끝음절은 조사(`은`·`는`·`이`·`가`)와 겹치지 않는 26자에서 고른다. 겹치면
`[가-힣]{2,4}(?:은|는|이|가)` 가 역추적해 `구성원` 만 잡고 별칭이 쪼개진다.
치환 후 캐지는 인물 수는 9명으로 치환 전과 같다.
...
- **실명 26개의 해시와 대조**한다. 코퍼스·케이스 픽스처·검색 평가셋·이 문서를 전부 훑어,
