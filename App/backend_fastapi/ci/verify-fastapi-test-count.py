...
# 2026-08-19: 932 -> 962. 이슈 셋을 병렬로 진행하고 병합한 뒤 정산한다.
#             -2 는 구조상 실패할 수 없던 meeting_eval 정렬 테스트(#636), +30 은 질의
#             로그(#627)다. 베이스라인 노트북(#625)은 테스트를 건드리지 않았다.
#
#             이번에는 정산을 별도 PR 로 빼지 않고 #636 안에 넣었다. 앞선 두 번처럼
#             "병합 후 정산" 으로 두면 순환이 생긴다 - #636 은 테스트를 지우는 이슈라
#             건수가 반드시 줄고, 하한이 높은 채로는 CI 가 빨개서 병합이 안 되며,
#             병합이 안 되면 정산할 기회가 오지 않는다. 하한을 내리는 정산은 그것을
#             유발한 브랜치 안에서 해야 한다. 올리는 정산만 뒤로 미룰 수 있다.
# 2026-08-20: 962 -> 981. 이번엔 병렬 이슈가 없어 브랜치 하나에서 바로 정산한다.
#             +12 는 평가 코퍼스 스냅샷 검사(test_corpus_snapshot.py)이고, 나머지 +7 은
#             #642·#646 이 병합되며 쌓인 미정산분이다. 하한이 실측보다 낮게 벌어져 있으면
#             그만큼 수집 누락을 못 잡으므로 같이 맞춘다.
MINIMUM_TESTS = 981
