...
```bash
bash scripts/run_ai_fastapi.sh
```
...
```bash
bash scripts/run_spring_backend.sh
```
...
bash scripts/build_spring_backend.sh
```
...
프론트는 `VITE_API_BASE_URL` 환경변수로 이 값을 주입한다 (`App/src/app/meetingAiApi.ts` 참고, 미설정 시 `http://localhost:8080/api/v1`로 폴백).
...
`WORKFLOW_CORS_ORIGINS` 환경변수에 콤마로 구분한 배포 프론트 도메인을 넣는다 (`backend-spring/src/main/java/com/workflowai/common/CorsConfig.java` 참고). 미설정 시 로컬 개발 주소(`http://localhost:5173,http://127.0.0.1:5173`)로 폴백하므로, 배포 환경에서는 반드시 재설정해야 한다. 프론트 배포 주소가 아직 없다면 임시로 `*`를 사용할 수 있다.
...
1. [Railway](https://railway.app) 로그인 후 **New Project → Deploy from GitHub repo** 선택, `rhantj/work-flow` 저장소와 `feature/meetings_ai` 브랜치를 연결한다.
2. 서비스 설정에서 **Root Directory**를 `backend-spring`으로 지정한다. Railway가 `backend-spring/Dockerfile`을 감지해 빌드한다 (`docker build -f backend-spring/Dockerfile backend-spring`과 동일한 빌드 컨텍스트).
