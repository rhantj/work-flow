...
| --- | --- | --- | --- |
| FS-1 인증/프로젝트 | - | - | - |
| FS-2 회의록/To-Do | 역할 추천(KNN, P1) | STT(Whisper), 문장 분류(KoBERT, P1) | 회의 요약·To-Do 추출(Function Calling) |
| FS-3 대시보드 | 지연 위험도·업무 편중(LightGBM) | 회의 감정/위험 신호(BERT, P2) | AI 추천 액션 문장 생성 |
| FS-4 Assistant | - | - | RAG 답변(Embedding + LLM) |
| FS-5 산출물 | - | - | 산출물 초안 생성(+RAG 옵션) |
| FS-6 업무보드/GitHub | - | 업무 우선순위 분류(BERT, P1) | - |
| FS-7 기여도/심사 | 이상치 탐지(Isolation Forest, P1) | - | 기여도 근거 요약 |
...
- **GitHub 연동(P1)**: REST API로 커밋/PR/Issue 수집, Webhook으로 갱신. PR ↔ 업무 ID 연결.
- **업무 우선순위 자동 추천(P1, DL)**: 업무 생성 시 제목/설명을 FS-6이 AI Backend `/ai/dl/task-priority`에 전달 → 높음/중간/낮음 자동 채움(사용자 수정 가능).
...
>
> **학습 데이터셋**: GitHub 활동 기반 기여도 → [GH Archive](https://www.gharchive.org/) (커밋·PR·Issue 이벤트 스트림)

### 핵심 결정사항
- **접근 제어**: 개인별 기여도 리포트·AI 평가 근거는 **심사자만**. 팀장/팀원 차단.
- **기여도 근거**: 업무 완료 + 회의 참석(FS-2 attendees) + GitHub 커밋/PR + 산출물 작성 집계.
