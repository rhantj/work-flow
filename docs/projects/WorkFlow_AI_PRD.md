...
> 아래 두 기능은 **계획했으나 구현하지 않았다.** 이 문서에 남은 관련 서술은 기획 시점의 범위 기록이다.
> - **산출물 생성** — 엔티티·리포지토리만 있고 API와 화면 진입점이 없다
> - **GitHub 연동** — 동일. 기여도 산정에도 GitHub 활동은 반영되지 않는다
>
> 프로젝트 생성 시 입력하는 **"목표 산출물"** 필드(`projects.deliverables`)는 위와 별개로 정상 동작한다.
...
4. 발표자료, 보고서, README, 제안서, 실험 보고서 등 산출물을 생성한다. *(미구현)*
...
| 산출물 생성 *(미구현)* | 생성/검토 | 생성/수정 | 제출물 조회 |
| GitHub 연동 *(미구현)* | 설정/조회 | 조회 | 조회 |
| 개인 코멘트 | 가능 | 가능 | 가능 |
| 팀 코멘트 | 가능 | 조회 | 가능 |
