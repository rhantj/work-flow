...
1. 로그인 화면의 "교수/심사자 데모 로그인" 버튼으로 심사자 역할로 진입한다(데모/테스트용, 정식 플로우는 회원가입 승인 기반).
2. 로그인 후 프로젝트 진입 화면에서 배정된 프로젝트를 확인하고 선택한다.
3. 팀별 진행률, 산출물, GitHub 활동, 회의록을 조회한다.
4. 개인별 기여도 리포트를 연다.
5. AI 평가 근거와 출처를 확인한다.
6. 개인/팀 코멘트를 작성한다.
7. 최종 평가 점수를 입력하고 공개 여부를 설정한다.

> 심사자 역할 진입 시 전역적으로 "열람 전용 모드"가 적용된다 — 기여도/평가 화면을 제외한 모든 화면에서 등록·수정·업로드 등 쓰기 액션이 비활성화되고, 화면 상단에 안내 배너가 표시된다.
...
| AC-07 | 심사자만 개인별 기여도 리포트와 AI 평가 근거를 볼 수 있으며, 그 외 화면에서는 열람 전용 모드로 쓰기 액션이 차단된다. |
