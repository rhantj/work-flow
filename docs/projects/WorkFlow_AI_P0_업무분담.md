...
> 산출물 생성 기능은 P0 범위에서 제외했다. 기존 산출물 오너(FS-5)는 ML/AI 모델링 전담으로 재배치했다. FS-7(구성원타) 이탈로 심사자/기여도/QA 업무는 구성원자(FS-4)로 통합했다.

## 1. 6인 기능 오너 (P0 재정의)

| 오너 | 이름 | 담당 기능 | 개발 범위 |
| --- | --- | --- | --- |
| FS-1 | 구성원다 | 인증/프로젝트/RBAC | 로그인, 프로젝트 생성, 팀원 초대, 역할 권한 |
| FS-2 | 구성원카 | 회의록 AI/To-Do | 업로드, AI 요약, To-Do 생성, 역할 배정 |
| FS-3 | 구성원라 | 대시보드/지연 위험도 | 진행률, 마감 임박, 업무량, 지연 위험도 예측 |
| FS-4 | 구성원자 | AI Assistant/RAG + 심사자/기여도/QA | 프로젝트 Q&A, embedding·vector 검색·출처 표시, 기여도 리포트, AI 평가 근거, 권한 테스트 |
| FS-5 | 구성원사 | ML/AI 모델링 | 업무 편중 점수, 지연 위험도 지원, 임베딩 파이프라인 |
| FS-6 | 구성원나 | 업무 보드 | 카테고리별 업무 UI, 칸반, 담당자/마감일 관리 |

## 2. P0 업무 분담표

| ID | P0 업무 | 담당(주) | 부담당 | 주요 기술 |
| --- | --- | --- | --- | --- |
| FR-01 | 인증/권한/마이페이지 (회원가입·로그인·RBAC·역할별 화면) | **구성원다 (FS-1)** | 구성원자 (FS-4, 권한 QA) | Spring Boot, JWT, RBAC, React |
| FR-02 | 프로젝트 관리 (생성·팀원 초대·마감일) | **구성원다 (FS-1)** | 구성원자 (FS-4) | Spring Boot, DB |
| FR-03 | 업무 보드 (상태별 칸반·카테고리 UI·담당자/마감일) | **구성원나 (FS-6)** | 구성원라 (FS-3) | React, Spring Boot |
| FR-04 | 회의록 업로드 (문서 업로드·텍스트 추출·AI 분석) | **구성원카 (FS-2)** | 구성원라 (FS-3) | LLM, File Parser |
| FR-05 | To-Do 자동 생성 (업무 추출·담당자 후보·미배정 처리) | **구성원카 (FS-2)** | 구성원나 (FS-6) | LLM, Function Calling |
| FR-06 | 대시보드 (진행률·마감 임박·업무량·최근 활동) | **구성원라 (FS-3)** | 구성원카 (FS-2) | DB Aggregation, Chart UI |
| AI-1 | ML 지연 위험도 예측 (정상/주의/위험) | **구성원라 (FS-3)** | 구성원카 (FS-2) | scikit-learn/LightGBM |
| FR-07 | ML 업무 편중 점수 (팀원별 과부하 점수) | **구성원사 (FS-5)** | 구성원나 (FS-6) | Regression/Classification |
| FR-08 | AI Assistant / RAG (프로젝트 맥락 Q&A·출처 표시) | **구성원자 (FS-4)** | 구성원사 (FS-5) | LLM, RAG, pgvector |
| FR-09 | 심사자 기여도 리포트 기본 (업무/회의/문서 기반) | **구성원자 (FS-4)** | 구성원다 (FS-1) | DB Aggregation, LLM |

## 3. 오너별 부하 요약

| 오너 | 이름 | 담당 기능(주) | 부담당 지원 |
| --- | --- | --- | --- |
| FS-1 | 구성원다 | FR-01, FR-02 | FR-06, FR-09 |
| FS-2 | 구성원카 | FR-04, FR-05 | FR-03 |
| FS-3 | 구성원라 | FR-06, AI-1 | FR-07 |
| FS-4 | 구성원자 | FR-08, FR-09 (+ QA 오너) | FR-04, FR-01, FR-02 |
| FS-5 | 구성원사 | FR-07 | AI-1, FR-08 |
| FS-6 | 구성원나 | FR-03 | FR-05 |
...
### FS-4 구성원자 · AI Assistant/RAG + 심사자/기여도/QA
...
- **QA 오너**: 전체 권한/시나리오 QA, 최종 권한 테스트·회귀 테스트 총괄. 심사자 권한 격리 부분은 구성원다(FS-1)이 부담당으로 지원.
...
- **ML 부하 분산**: 지연 위험도·업무 편중 등 ML이 구성원나(FS-3) 한 명에게 몰리지 않도록 구성원사(FS-5)가 모델링을 분담.
- **데이터 흐름 교차 배정**: 회의록→To-Do→보드→대시보드 흐름상 인접 오너를 부담당으로 교차 배치 (FS-2↔FS-6, FS-3↔FS-1).
- **LLM 파이프라인 공유**: 회의록 분석(FR-04)·RAG(FR-08)는 LLM/임베딩 로직을 공유하므로 구성원자(FS-4)·구성원사(FS-5)가 상호 부담당.
- **FS-7 → FS-4 통합**: 심사자 기여도 리포트(FR-09)·QA 오너 역할을 구성원자(FS-4)로 통합. 기여도 요약이 RAG의 "출처 포함 요약"과 로직을 공유해 중복 개발이 없고, RAG 오너가 자연스럽게 인계.
- **권한/QA**: 권한 성격이 강한 FR-01·FR-02는 QA 오너 구성원자(FS-4)가 부담당으로 검증. 심사자 RBAC 격리는 인증 오너 구성원다(FS-1)이 FR-09 부담당으로 지원.
